...
contract DemoRelayFeeTest is Test {
  ...
  uint256 internal constant TOKEN_FEE = 1100e18;
  uint256 internal constant TOKEN_NET = 9900e18;
  ...
  function test_demo() public {
    ...
    DeployRelayPoc.Deployment memory d = deployer.deployForTest(
      DeployRelayPoc.Actors({
        deployer: address(deployer), manager: manager, keeper: keeper, strategist: strategist, treasury: treasury
      })
    );
    ...
    assertTrue(IERC20(d.token).transfer(alice, 1000e18), 'Alice TOKEN funding failed');
    vm.startPrank(alice);
    IERC20(d.token).approve(d.votingEscrow, 1000e18);
    uint256 tokenId = relay.VOTING_ESCROW().createStake(1000e18, 0, true);
    relay.VOTING_ESCROW().setApprovalForAll(address(relay), true);
    relay.VOTING_ESCROW().setApprovalForAll(d.vpm, true);
    relay.requestDeposit(tokenId, 1000e18, alice);
    vm.stopPrank();
    vm.prank(keeper);
    ...
    vm.prank(address(deployer));
    assertTrue(IERC20(d.token).transfer(d.compounderRelay, TOKEN_GROSS), 'compounder TOKEN funding failed');
    vm.prank(keeper);
